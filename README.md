...
`model/weights.npz`, `model/vocab.json`, and `model/metrics.json`.
...
The app is deployed as a Docker container on
[Hugging Face Spaces](https://huggingface.co/spaces) (free tier). The image
(see [`Dockerfile`](Dockerfile)) only installs `requirements.txt`. TensorFlow
is a training-time dependency and never ships to production.
